...
exports.setupAuthTimeout = setupAuthTimeout;
    ...
    try {
        ...
        switch (data.type) {
            ...
            case 'create_room': {
                ...
                roomManager.createRoom(data.name, data.rules, socket, socket.playerId, socket.telegramUser);
                break;
            }
            case 'join_room': {
                if (!data.roomId) {
                ...
                roomManager.joinRoom(data.roomId, socket, socket.playerId, socket.telegramUser);
                break;
            }
            case 'leave_room': {
...
function handleAuthentication(socket, data) {
    try {
        ...
        // Валидация токена
        const tokenPayload = TelegramAuth_1.TelegramAuth.validateAuthToken(token);
        if (!tokenPayload) {
...
function setupAuthTimeout(socket) {
